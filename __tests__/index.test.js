...
describe("to test the input validation ", () => {
  ...
  test("input validation with invalid inputs", () => {
    ...
    const symbolText = chance.string({ pool: "!@#$%^&*()_+=[]{}|;:<>?/~" });
...
describe("to check that complete functionality working properly", () => {
  ...
  test("to check the uncompleted task got completed when clicking complete button", () => {
    ...
    const taskNameContainer = taskName.parentElement;
    opacityExists = taskNameContainer.classList.contains("opacity");
    taskName = getByText(todoList, validInput);
    todoCard = taskName.parentElement.parentElement.parentElement; //span -> .task-name -> .task-name-container -> .todo-card
...
describe("to test that ui individual functions working properly", () => {
  ...
  test("to test that given value inside the task name container", () => {
    ...
    const taskNameContainer = createTaskName(task.taskName, true);
    expect(getByText(taskNameContainer, task.taskName).textContent).toBe(
      task.taskName
    );
  });
  ...
  test("to check that delete task when invalid inputs recieved", () => {
    ...
    const text = getByText(taskCard, task.taskName);
